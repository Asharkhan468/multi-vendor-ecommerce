...
export default function AdminOrdersComponent({ orders }: { orders?: any[] }) {
 ...
 const fetchOrders = async () => {
  ...
  if (res.success) {
    setDemoOrders(res.data.orders); // raw data
  } else {
    toast.error("Orders not fetched");
  }
};
  ...
  const data = orders && orders.length ? orders : formattedData;
  ...
  return (
    <section className="w-full p-4 md:p-6">
      ...
      <header className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Quick snapshot of recent orders.
          </p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 p-4 bg-white rounded-2xl shadow-sm border">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-sm text-gray-600">
              Showing last {chartData.length} days
            </div>
          </div>
        ...
        <div
          className="p-4 bg-white rounded-2xl shadow-sm border overflow-y-auto"
          style={{ maxHeight: 360 }}
        >
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (s === "Delievered") bg = "bg-emerald-200 text-emerald-800";
